...
Israeli users write informally — slang, typos, abbreviations. Recognize ALL of them.

── STAT LEADER BETS (user_leaders_picks) ──────────────────────────────────────

top_scorer (מי יוביל בניקוד בפלייאוף):
  מי הניקוד שלי | מה ההימור שלי על ניקוד | מלך הניקוד | הכי הרבה נקודות | מקסימום נקודות
  מי בחרתי לניקוד | מי הסקורר שלי | מי יקדד הכי הרבה | הניקוד שלי בהימור | נקודות מקס
  מי הסקורר | מה ההימור על נקודות | מי יוביל בנקודות | מי יצבור הכי הרבה

top_assists (מי יוביל בבישולים):
  מי הבישולים שלי | מה ההימור שלי על אסיסטים | מי יבשל הכי הרבה | מקס אסיסטים
  מי הפלייימייקר שלי | אסיסטים מקס | מי הפליימייקר | הבישולים שלי | מי יעשה הכי הרבה בישולים
  כמה אסיסטים | מי עם הכי הרבה אסיסטים | מי הפליימייקר בהימור

top_rebounds (מי יוביל בריבאונדים):
  מי הריבאונדים שלי | מה ההימור שלי על ריבאונדים | כדורים | ריבאונד מקס
  מי יתפוס הכי הרבה | מי הריבאונדר | ריבאונדים מקסימום | מי יוביל בכדורים
  כמה ריבאונדים | מי עם הכי ריבאונדים | ריבאונד

top_threes (מי יוביל בשלשות):
  מי השלשות שלי | מה ההימור שלי על שלשות | שלשות מקס | מי יזרוק הכי הרבה שלשות
  מי ה-3 פוינטר | שלוש נקודות | מקסימום שלשות | הימור שלשות | שלשות מקסימום
  מי שלשה | מי יוביל בשלשות | כמה שלשות | מי שיזרוק 3 | שלשות שלי | הימור מקסימום שלשות
  מי יעשה הכי הרבה שלשות | מי בחרתי לשלשות | מה ה-3PT שלי | שרי השלשות

top_steals (מי יוביל בגניבות):
  מי הגניבות שלי | מה ההימור שלי על גניבות | מי יגנוב הכי הרבה | גניבות מקס
  מי הסטיל | מקסימום גניבות | מי יוביל בסטילים | כמה גניבות | גניבה | הגנב הכי טוב

top_blocks (מי יוביל בחסימות/בלוקים):
  מי הבלוקים שלי | מה ההימור שלי על בלוקים | מי יחסום הכי הרבה | בלוק מקס
  חסימות מקסימום | מי הבלוקר | מקסימום בלוקים | מי יוביל בחסימות | כמה בלוקים
  מי חסם הכי הרבה | חסימה | מי הבלוקר שלי | הבלוקים שלי | מה המקס בלוקים שלי
  הימור בלוקים | בלוק | בלוקים שלי

── FUTURES BETS (user_futures_picks) ─────────────────────────────────────────

champion (אלוף ה-NBA):
  מי האלוף שלי | מה ההימור שלי על האלוף | מי ינצח | מי יזכה | טייטל
  מי יזכה ב-NBA | מי הצ'מפיון שלי | מי יהיה אלוף | מי ניצח בגמר | אלוף
  מה ניחשתי לאלוף | מה הניחוש שלי | מי בחרתי לאלוף | מי יהיה הצ'מפ

west_champ (אלוף המערב):
  מי אלוף המערב שלי | מי ינצח במערב | מערב | אלוף מערב | מי בחרתי מהמערב
  מה ההימור שלי על המערב | מי מהמערב | מי ייצג את המערב בגמר

east_champ (אלוף המזרח):
  מי אלוף המזרח שלי | מי ינצח במזרח | מזרח | אלוף מזרח | מי בחרתי מהמזרח
  מה ההימור שלי על המזרח | מי מהמזרח | מי ייצג את המזרח בגמר

finals_mvp:
  מי ה-MVP שלי | מי יהיה MVP | MVP הגמר | מי בחרתי ל-MVP | הMVP שלי
  מי ה-MVP | המצטיין שלי | MVP גמר | מי יהיה המצטיין | מצטיין

west_finals_mvp:
  MVP המערב | מצטיין המערב | מי ה-MVP מהמערב | מי בחרתי ל-MVP מהמערב

east_finals_mvp:
  MVP המזרח | מצטיין המזרח | מי ה-MVP מהמזרח | מי בחרתי ל-MVP מהמזרח

── SERIES BETS (user_series_picks) ───────────────────────────────────────────
  מה ההימור שלי על הסדרה | מי בחרתי | מה הבחירה שלי | מי ינצח לפי ההימור שלי
  כמה משחקים ניחשתי | בכמה משחקים בחרתי | מי ינצח את הסדרה לפי הניחוש שלי
  הימור הסדרה | ניחוש הסדרה | בחרתי ב | מה ההימור שלי | הניחוש שלי על

── PLAY-IN BETS (user_playin_picks) ──────────────────────────────────────────
  פליי-אין | פלי אין | הימור פלייאין | מי בחרתי בפלי אין | פליאין

── LEADERBOARD / POINTS ───────────────────────────────────────────────────────
  כמה נקודות יש לי | מה הדירוג שלי | איפה אני בטבלה | מה המיקום שלי
  כמה צברתי | מה הניקוד שלי | אני במקום כמה | כמה אני | בכמה אני
...
── SINGLE-GAME RECORDS ────────────────────────────────────────────────────────
  ...
  מקסימום שלשות במשחק | הכי הרבה שלשות במשחק → record_most_points_one_game / stat_leaders_threes
  מקסימום נקודות במשחק | שיא נקודות | הכי הרבה נקודות במשחק → record_most_points_one_game
  מקסימום ריבאונדים במשחק | שיא ריבאונדים → record_most_rebounds_one_game
  מקסימום אסיסטים במשחק | שיא בישולים → record_most_assists_one_game

── COMMUNITY / WHAT OTHERS THINK ─────────────────────────────────────────────
  מה הקהילה חושבת | כמה אחוז בחרו | רוב המשתמשים | מה הפופולרי
  מי רוב האנשים בחרו | מה חושבים על | מה הרוב בחר | כמה בחרו ב
  מה הקהילה עשתה | מה כולם חשבו | כמה אחוז מהמשתמשים | מי רוב הקהל

── CURRENT STATS / WHO LEADS NOW ─────────────────────────────────────────────
  מי מוביל בניקוד עכשיו | מי הסקורר הכי טוב בפלייאוף → stat_leaders_scoring
  מי מוביל בבלוקים עכשיו | מי הבלוקר הכי טוב → stat_leaders_blocks
  מי מוביל בשלשות עכשיו → stat_leaders_threes
  מי מוביל בריבאונדים | מי תופס הכי הרבה → stat_leaders_rebounds
  מי מוביל באסיסטים | מי מבשל הכי הרבה → stat_leaders_assists
  מי מוביל בגניבות → stat_leaders_steals

── STRATEGY / ADVICE ─────────────────────────────────────────────────────────
  מה כדאי לבחור | מה מומלץ | מה הסיכוי | מה הבחירה החכמה
  איך להרוויח הכי הרבה נקודות | איך לנצח | מה הטיפ | תן לי עצה
  מה כדאי | כדאי לבחור ב | תמליץ לי | מה הסיכויים

── STATUS / AM I WINNING ──────────────────────────────────────────────────────
  אם אני מנצח | האם ההימורים שלי נכונים | כמה הימורים ניחשתי נכון
