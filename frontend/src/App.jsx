          ...
          {sorted.map((user) => {
            ...
            return (
              <div key={user.rank} className={`transition-colors ${rowBg} ${leftBar}`}>
                ...
                {/* ── Provisional breakdown panel — compact ── */}
                {provPopover === user.user_id && hasProvBreakdown && (
                  <div className="mx-2 mb-2 rounded-xl overflow-hidden border border-amber-400/35 bg-slate-900/95 shadow-lg">

                    {/* Header — single tight row */}
                    <div className="flex items-center justify-between px-3 py-2 border-b border-amber-400/15">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm">⚡</span>
                        <span className="text-xs font-black text-amber-300">Provisional Pts</span>
                        <span className="text-[9px] text-slate-600">· finalizes when series end</span>
                      </div>
                    ...
                    {Object.keys(provSeriesBD).length > 0 && (
                      <div className="px-2.5 pt-2 pb-1">
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-600 mb-1.5">🏆 Series Bets</p>
                        <div className="space-y-1.5">
                          {Object.entries(provSeriesBD).map(([key, info]) => {
                            ...
                            return (
                              <div key={key} className="bg-slate-800/60 border border-slate-700/50 rounded-lg overflow-hidden">

                                {/* Round label + total pts — compact */}
                                <div className="flex items-center justify-between px-2.5 pt-1.5 pb-1">
                                  <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">{info.label}</span>
                                  <span className="text-[11px] font-black text-amber-300">+{info.pts}</span>
                                </div>

                                {/* Winner pick — inline single row */}
                                {info.winner_pts > 0 && (
                                  <div className="mx-2 mb-1.5">
                                    <div className="flex items-center bg-green-500/10 border border-green-500/25 rounded-lg px-2 py-1.5">
                                      <span className="text-green-400 text-[10px] font-black mr-1">✓</span>
                                      <span className="text-xs font-black text-green-300">{info.picked_abbr}</span>
                                      <span className="text-sm font-black text-white mx-1.5">{info.pick_wins}</span>
                                      <span className="text-[10px] font-bold text-slate-600">–</span>
                                      <span className="text-sm font-black text-slate-500 mx-1.5">{info.opp_wins}</span>
                                      <span className="text-xs font-black text-slate-500">{info.opp_abbr || '—'}</span>
                                      <div className="ml-auto shrink-0 flex items-center gap-1 bg-green-500/15 border border-green-500/25 rounded px-1.5 py-0.5">
                                        <span className="text-[8px] font-black text-green-400 uppercase">lead</span>
                                        <span className="text-[10px] font-black text-amber-300">+{info.winner_pts}</span>
                                      </div>
                                    </div>
                                  </div>
                                )}

                                {/* Leader bets — ultra-compact rows */}
                                {leaderEntries.length > 0 && (
                                  <div className={`mx-2 space-y-1 ${info.winner_pts > 0 ? 'border-t border-slate-700/40 pt-1.5' : ''} pb-2`}>
                                    {leaderEntries.map(([cat, linfo]) => {
                                      const lastName = linfo.player?.split(' ').slice(-1)[0] ?? '';
                                      return (
                                        <div key={cat} className="flex items-center gap-1.5 px-2 py-1 bg-cyan-500/5 border border-cyan-500/15 rounded-md">
                                          <span className="text-[9px] font-black text-cyan-500 w-6 shrink-0">{catLabel[cat]}</span>
                                          <span className="text-[11px] font-black text-cyan-300 flex-1 min-w-0 truncate">
                                            {lastName}{linfo.value != null ? <span className="text-white"> ({linfo.value})</span> : ''}
                                          </span>
                                          <span className="text-[9px] text-green-500 shrink-0">✓lead</span>
                                          <span className="text-[11px] font-black text-amber-300 shrink-0">+{linfo.pts}</span>
                                        </div>
                                      );
                                    })}
                                  </div>
                    ...
                    {Object.keys(provBreakdownNorm).length > 0 && (
                      <div className="px-2.5 pt-2">
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-600 mb-1.5">📊 Max-Stat Bets</p>
                        <div className="space-y-1">
                          {Object.entries(provBreakdownNorm).map(([cat, info]) => {
                            ...
                            const catLabel = { scorer: '🏀 Pts', assists: '🎯 Ast', rebounds: '💪 Reb', threes: '3️⃣ 3s', steals: '🤚 Stl', blocks: '🚫 Blk' }[cat] || cat;
                            const exact = predicted === record;
                            return (
                              <div key={cat} className="flex items-center gap-2 bg-amber-500/8 border border-amber-400/15 rounded-lg px-2.5 py-1.5">
                                <span className="text-[10px] font-black text-slate-300 w-14 shrink-0">{catLabel}</span>
                                {predicted != null && record != null ? (
                                  <div className="flex items-center gap-2 flex-1 min-w-0">
                                    <span className="text-[9px] text-slate-500">Bet <span className="text-white font-bold">{predicted}</span></span>
                                    <span className="text-slate-600">·</span>
                                    <span className="text-[9px] text-slate-500">Record <span className={`font-bold ${exact ? 'text-green-400' : 'text-cyan-400'}`}>{record}{exact ? ' ✓' : ''}</span></span>
                                  </div>
                                ) : <div className="flex-1" />}
                                <span className="text-xs font-black text-amber-300 shrink-0">+{catPts}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {/* Footer */}
                    <div className="px-3 pt-2 pb-2.5 flex items-center justify-between">
                      <span className="text-[9px] text-slate-600">⏳ Finalizes when series end</span>
